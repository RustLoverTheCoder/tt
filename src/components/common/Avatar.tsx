...
import "./Avatar.scss";
import { loadFullUser } from "../../global/actions";
...
const Avatar: FC<OwnProps> = ({
  ...
  const shouldShowVideo =
    isIntersecting &&
    withVideo &&
    (shouldShowPhotoVideo || shouldShowUserVideo);
  const profilePhoto =
  ...
  const shouldLoadVideo =
    shouldShowVideo && (hasProfileVideo || shouldShowPhotoVideo);
  ...
  const shouldPlayVideo = Boolean(
    isIntersecting && videoBlobUrl && shouldLoadVideo
  );
  ...
  if (isSavedMessages) {
    content = (
      <i
        className={clsx(icon, "icon-avatar-saved-messages")}
        role="img"
        aria-label={author}
      />
    );
    ...
    content = (
      <i
        className={clsx(icon, "icon-avatar-deleted-account")}
        role="img"
        aria-label={author}
      />
    );
    ...
    content = (
      <i
        className={clsx(icon, "icon-reply-filled")}
        role="img"
        aria-label={author}
      />
    );
      ...
      <>
        ...
        {shouldPlayVideo && (
          <OptimizedVideo
            canPlay
            src={videoBlobUrl}
            className={clsx(media, "avatar-media", "poster")}
            muted
