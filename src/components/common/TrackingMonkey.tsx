...
const TrackingMonkey: FC<OwnProps> = ({
  ...
  const trackningFramesPerSymbol =
    (TRACKING_END_FRAME - TRACKING_START_FRAME) / codeLength;
  const stickerSize = isMobile ? STICKER_SIZE_AUTH_MOBILE : STICKER_SIZE_AUTH;

  const handleFirstMonkeyLoad = useCallback(
    () => setIsFirstMonkeyLoaded(true),
    []
  );

  function getTrackingFrames(): [number, number] {
    const startFrame =
      (code && code.length > 1) || trackingDirection < 0
  ...
  return (
    <div id="monkey" className={isBig ? "big" : ""}>
      {!isFirstMonkeyLoaded && <div className="monkey-preview" />}
      <AnimatedSticker
