import React, { memo, useCallback, useEffect, useRef, FC } from "react";
...
const Spoiler: FC<OwnProps> = ({ children, messageId }) => {
  ...
  const handleClick = useCallback(
    (e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
      ...
      actionsByMessageId
        .get(messageId!)
        ?.forEach((actions) => actions.reveal());
      ...
      const readingMs =
        Math.round(totalContentLength / READING_SYMBOLS_PER_SECOND) * 1000;
      const timeoutMs = Math.max(
        MIN_HIDE_TIMEOUT,
        Math.min(readingMs, MAX_HIDE_TIMEOUT)
      );

      setTimeout(() => {
        actionsByMessageId
          .get(messageId!)
          ?.forEach((actions) => actions.conceal());
        conceal();
      }, timeoutMs);
    },
    [conceal, messageId]
  ...
  useEffect(() => {
    ...
    if (actionsByMessageId.has(messageId)) {
      actionsByMessageId
        .get(messageId)!
        .push({ reveal, conceal, contentLength });
    } else {
  ...
  return (
    ...
    >
      <span className={clsx("content")} ref={contentRef}>
        {children}
      </span>
    </span>
  );
