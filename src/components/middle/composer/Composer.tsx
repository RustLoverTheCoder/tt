...
import { loadScheduledHistory, loadSendAs, showNotification, showDialog, sendMessage, clearDraft, forwardMessages, callAttachBot, sendInlineBotResult, resetOpenChatWithDraft, closePollModal, openChat, showAllowedMessageTypesNotification, openPollModal, addRecentCustomEmoji, addRecentEmoji } from "../../../global/actions";
...
type ScheduledMessageArgs =
  | TabState["contentToBeScheduled"]
  | {
      id: string;
      queryId: string;
      isSilent?: boolean;
...
const Composer: FC<OwnProps & StateProps> = ({
  ...
  const canShowSendAs =
    sendAsPeerIds &&
    (sendAsPeerIds.length > 1 ||
      !sendAsPeerIds.some((peer) => peer.id === currentUserId!));
  // Prevent Symbol Menu from closing when calendar is open
  const [isSymbolMenuForced, forceShowSymbolMenu, cancelForceShowSymbolMenu] =
    useFlag();
  ...
  useSyncEffect(
    ([prevChatId, prevSendAsPeerIds]) => {
      // We only animate send-as button if `sendAsPeerIds` was missing when opening the chat
      shouldAnimateSendAsButtonRef.current = Boolean(
        chatId === prevChatId && sendAsPeerIds && !prevSendAsPeerIds
      );
    },
  ...
  const {
    ...
    () => getAllowedAttachmentOptions(chat, isChatWithBot),
    [chat, isChatWithBot]
  );
  ...
  const [isBotCommandMenuOpen, openBotCommandMenu, closeBotCommandMenu] =
    useFlag();
  ...
  const {
    ...
    Boolean(
      isReady && isForCurrentMessageList && !hasAttachments && lastSyncTime
    ),
    chatId,
    getHtml,
    inlineBots
  ...
  const insertHtmlAndUpdateCursor = useCallback(
    (newHtml: string, inputId: string = EDITABLE_INPUT_ID) => {
      if (inputId === EDITABLE_INPUT_ID && isComposerBlocked) return;
      const selection = window.getSelection()!;
      let messageInput: HTMLDivElement;
      if (inputId === EDITABLE_INPUT_ID) {
        messageInput = document.querySelector<HTMLDivElement>(
          EDITABLE_INPUT_CSS_SELECTOR
        )!;
      } else {
  ...
  const insertCustomEmojiAndUpdateCursor = useCallback(
    (emoji: ApiSticker, inputId: string = EDITABLE_INPUT_ID) => {
  ...
  const [handleEditComplete, handleEditCancel, shouldForceShowEditing] =
    useEditing(
      getHtml,
      setHtml,
      editingMessage,
  ...
  const validateTextLength = useCallback(
    (text: string, isAttachmentModal?: boolean) => {
  ...
  const checkSlowMode = useCallback(() => {
    if (slowMode && !isAdmin) {
      ...
      const nextSendDateNotReached =
        slowMode.nextSendDate && slowMode.nextSendDate > nowSeconds;

      if (
        (secondsSinceLastMessage &&
          secondsSinceLastMessage < slowMode.seconds) ||
        ...
        showDialog({
          data: {
            message: lang(
              "SlowModeHint",
              formatMediaDuration(secondsRemaining)
            ),
            isSlowMode: true,
            hasErrorKey: false,
          },
  ...
  const sendAttachments = useCallback(
    ...
    [
      attachmentSettings.shouldCompress,
  ...
  const handleSend = useCallback(
    async (isSilent = false, scheduledAt?: number) => {
      if (connectionState !== "connectionStateReady") {
        return;
      }
      ...
      if (activeVoiceRecording) {
        ...
        if (record) {
          ...
          currentAttachments = [
            await buildAttachment(VOICE_RECORDING_FILENAME, blob, {
  ...
  const handleMessageSchedule = useCallback(
    (args: ScheduledMessageArgs, scheduledAt: number) => {
      ...
      if (!args || Object.keys(restArgs).length === 0) {
        ...
        args.sendCompressed !== undefined ||
        args.sendGrouped !== undefined
      ) {
  ...
  useEffectWithPrevDeps(
    ([prevContentToBeScheduled]) => {
      if (
        contentToBeScheduled &&
        contentToBeScheduled !== prevContentToBeScheduled
      ) {
  ...
  const handleCustomEmojiSelect = useCallback(
    (emoji: ApiSticker, inputId?: string) => {
    ...
    [
      insertCustomEmojiAndUpdateCursor,
      isChatWithSelf,
      isCurrentUserPremium,
      showCustomEmojiPremiumNotification,
    ]
  ...
  const handleCustomEmojiSelectAttachmentModal = useCallback(
    (emoji: ApiSticker) => {
  ...
  const handleGifSelect = useCallback(
    (gif: ApiVideo, isSilent?: boolean, isScheduleRequested?: boolean) => {
  ...
  const sendSilent = useCallback(
    (additionalArgs?: ScheduledMessageArgs) => {
      if (shouldSchedule) {
        requestCalendar((scheduledAt) => {
          handleMessageSchedule(
        ...
        additionalArgs &&
        ("sendCompressed" in additionalArgs || "sendGrouped" in additionalArgs)
      ) {
  ...
  const insertTextAndUpdateCursor = useCallback(
    (text: string, inputId: string = EDITABLE_INPUT_ID) => {
  ...
  const insertTextAndUpdateCursorAttachmentModal = useCallback(
    (text: string) => {
  ...
  const areVoiceMessagesNotAllowed =
  ...
  const handleSendScheduledAttachments = useCallback(
    (sendCompressed: boolean, sendGrouped: boolean) => {
      requestCalendar((scheduledAt) => {
  ...
  const handleSendSilentAttachments = useCallback(
    (sendCompressed: boolean, sendGrouped: boolean) => {
  ...
  const onSend =
    mainButtonState === MainButtonState.Edit
      ? handleEditComplete
      : mainButtonState === MainButtonState.Schedule
      ? handleSendScheduled
      : handleSend;

  const withBotMenuButton =
    isChatWithBot && botMenuButton?.type === "webApp" && !editingMessage;
  const isBotMenuButtonOpen = useDerivedState(() => {
    return withBotMenuButton && !getHtml() && !activeVoiceRecording;
  }, [withBotMenuButton, getHtml, activeVoiceRecording]);

  const withBotCommands =
    isChatWithBot &&
    botMenuButton?.type === "commands" &&
    !editingMessage &&
    botCommands !== false &&
    !activeVoiceRecording;
    ...
    <div className={className}>
      {canAttachMedia && isReady && (
        <DropArea
          ...
          withQuick={
            dropAreaState === DropAreaState.QuickFile ||
            prevDropAreaState === DropAreaState.QuickFile
          }
          onHide={onDropHide}
      ...
      <div id="message-compose">
        ...
        <ComposerEmbeddedMessage
          ...
          shouldForceShowEditing={Boolean(
            shouldForceShowEditing && editingMessage
          )}
        />
        <WebPagePreview
          chatId={chatId}
        ...
        <div className="message-input-wrapper">
          ...
          {withBotCommands && (
            <ResponsiveHoverButton
              className={clsx(
                "bot-commands",
                isBotCommandMenuOpen && "activated"
              )}
              round
          ...
          {canShowSendAs && (sendAsUser || sendAsChat) && (
            <Button
              ...
              color="translucent"
              onClick={
                isSendAsMenuOpen ? closeSendAsMenu : handleSendAsMenuOpen
              }
            ...
            >
              <Avatar user={sendAsUser} chat={sendAsChat} size="tiny" />
            </Button>
          )}
          ...
          <MessageInput
            ...
            placeholder={
              activeVoiceRecording &&
              windowWidth <= SCREEN_WIDTH_TO_HIDE_PLACEHOLDER
                ? ""
                : !isComposerBlocked
          ...
          {activeVoiceRecording && Boolean(currentRecordTime) && (
            <span className="recording-state">
              {formatVoiceRecordDuration(
                //@ts-ignore
                currentRecordTime - startRecordTimeRef.current!
              )}
            </span>
          )}
          <AttachMenu
...
//       const messageWithActualBotKeyboard =
//         (isChatWithBot || !isChatWithUser) &&
//         selectNewestMessageWithBotKeyboardButtons(global, chatId, threadId);
//       const scheduledIds = selectScheduledIds(global, chatId, threadId);
//       const { language, shouldSuggestStickers, shouldSuggestCustomEmoji } =
...
//           : undefined;
//       const botKeyboardMessageId = messageWithActualBotKeyboard
//         ? messageWithActualBotKeyboard.id
//         : undefined;
//       const keyboardMessage = botKeyboardMessageId
//         ? selectChatMessage(global, chatId, botKeyboardMessageId)
//         : undefined;
//       const { currentUserId } = global;
//       const defaultSendAsId = chat?.fullInfo
//         ? chat?.fullInfo?.sendAsId || currentUserId
//         : undefined;
//       const sendAsId =
//         chat?.sendAsPeerIds &&
//         defaultSendAsId &&
...
//           ? defaultSendAsId
//           : chat?.adminRights?.anonymous
//           ? chat?.id
//           : undefined);
//       const sendAsUser = sendAsId ? selectUser(global, sendAsId) : undefined;
//       const sendAsChat =
//         !sendAsUser && sendAsId ? selectChat(global, sendAsId) : undefined;
//       const requestedDraftText = selectRequestedDraftText(global, chatId);
//       const requestedDraftFiles = selectRequestedDraftFiles(global, chatId);
//       const currentMessageList = selectCurrentMessageList(global);
//       const isForCurrentMessageList =
//         chatId === currentMessageList?.chatId &&
//         threadId === currentMessageList?.threadId &&
//         messageListType === currentMessageList?.type;
//       const user = selectUser(global, chatId);
//       const canSendVoiceByPrivacy =
//         (user && !user.fullInfo?.noVoiceMessages) ?? true;
...
//       return {
//         editingMessage: selectEditingMessage(
//           global,
//           chatId,
//           threadId,
//           messageListType
...
//         withScheduledButton:
//           messageListType === "thread" && Boolean(scheduledIds?.length),
...
//         botCommands: chatBot?.fullInfo
//           ? chatBot.fullInfo.botInfo?.commands || false
//           : undefined,
