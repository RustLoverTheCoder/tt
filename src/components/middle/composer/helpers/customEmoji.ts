...
export function buildCustomEmojiHtmlFromEntity(
  rawText: string,
  entity: ApiMessageEntityCustomEmoji
) {
  const [customEmojis] = useAtom(customEmojisAtom);
  const customEmoji = customEmojis.byId[entity.documentId];
...
export function getCustomEmojiSize(
  maxEmojisInLine?: number
): number | undefined {
