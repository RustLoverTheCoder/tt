...
import { loadSeenBy, loadReactors, loadStickers, loadFullChat, setReplyingToId, openChat, setEditingId, pinMessage, openForwardMenu, faveSticker, unfaveSticker, cancelPollVote, toggleMessageSelection, sendScheduledMessages, rescheduleMessage, openSeenByModal, openReactorListModal, copyMessagesByIds, cancelMessageMediaDownload, downloadMessageMedia, saveGif, toggleReaction, requestMessageTranslation, showOriginalMessage, openMessageLanguageModal, closePoll } from "../../../global/actions";
import { getGlobal } from "../../../lib/Reactn";
...
const ContextMenuContainer: FC<OwnProps & StateProps> = ({
  ...
  const seenByRecentUsers = useMemo(() => {
    if (message.reactions?.recentReactions?.length) {
      ...
      const uniqueReactors = new Set(
        message.reactions?.recentReactions?.map(
  ...
  const isDownloading = album
    ? album.messages.some((msg) => activeDownloads.includes(msg.id))
  ...
  const handleCopyLink = useCallback(() => {
    copyTextToClipboard(
      getChatMessageLink(message.chatId, chatUsername, threadId, message.id)
    );
    closeMenu();
  ...
  return (
    <div
      className={clsx("ContextMenuContainer", transitionClassNames)}
    >
      <MessageContextMenu
