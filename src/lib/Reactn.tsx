...
import arePropsShallowEqual, {
  getUnequalProps,
} from "../util/arePropsShallowEqual";
...
type GlobalState = AnyLiteral & { DEBUG_capturedId?: number };
...
type Actions = Record<
  ActionNames,
  (payload?: ActionPayload, options?: ActionOptions) => void
>;
...
export function setGlobal(newGlobal?: GlobalState, options?: ActionOptions) {
  if (typeof newGlobal === "object" && newGlobal !== currentGlobal) {
    if (DEBUG) {
      if (
        newGlobal.DEBUG_capturedId &&
        newGlobal.DEBUG_capturedId !== DEBUG_currentCapturedId
      ) {
...
function updateContainers() {
  ...
  for (const container of containers.values()) {
    ...
    try {
      newMappedProps = mapStateToProps(
        currentGlobal,
        ownProps,
        detachWhenChanged
      );
    ...
    if (
      Object.keys(newMappedProps).length &&
      !arePropsShallowEqual(mappedProps!, newMappedProps)
    ) {
      if (DEBUG_MORE) {
        // eslint-disable-next-line no-console
        console.log(
  ...
  if (DEBUG) {
    ...
    if (updateTime > 7) {
      // eslint-disable-next-line no-console
      console.warn(
        `[TeactN] Slow containers update: ${Math.round(updateTime)} ms`
      );
    }
  }
}
...
export function withGlobal<OwnProps extends AnyLiteral>(
  ...
  return (Component: FC) => {
    return function TeactNContainer(props: OwnProps) {
      (TeactNContainer as FC_withDebug).DEBUG_contentComponentName =
        Component.name;
      ...
      if (
        ...
        if (!container.isDetached) {
          try {
            container.mappedProps = mapStateToProps(
              currentGlobal,
              props,
              container.detachWhenChanged
            );
          } catch (err: any) {
            handleError(err);
          }
...
export function typify<ProjectGlobalState, ActionPayloads>() {
  ...
  type ProjectActions = {
    [ActionName in ProjectActionNames]: undefined extends ActionPayloads[ActionName]
...
if (DEBUG) {
  ...
  document.addEventListener("dblclick", () => {
    ...
    console.warn(
      "GLOBAL CONTAINERS",
      orderBy(
        Array.from(containers.values()).map(
          ({ DEBUG_componentName, DEBUG_updates }) => ({
            DEBUG_componentName,
            DEBUG_updates,
          })
        ),
