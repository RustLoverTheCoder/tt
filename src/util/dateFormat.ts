...
const MONTHS_FULL_LOWERCASE: any = MONTHS_FULL.map((month) =>
  month.toLowerCase()
);
...
export function formatCountdown(lang: LangFn, msLeft: number) {
...
export function formatCountdownShort(lang: LangFn, msLeft: number): string {
  if (msLeft < 60 * 1000) {
    ...
    return lang(
      ...
      Math.ceil(msLeft / MILLISECONDS_IN_DAY)
    );
  }
}
...
export function formatTimeDuration(
  ...
  labels.forEach((label, idx) => {
    ...
    const modulus =
      labels[idx === labels.length - 1 ? idx : idx + 1].multiplier!;
...
export function formatHumanDate(
  ...
  if (!noWeekdays) {
    if (toIsoString(date) === toIsoString(today)) {
      return (isUpperFirst || !isShort ? upperFirst : lowerFirst)(
        lang("Weekday.Today")
      );
    }
...
export function parseDateString(query = ""): string | undefined {
  ...
  if (matchNumberDate) {
    ...
    return !(year && year < MIN_SEARCH_YEAR) &&
      isValidDate(date, month - 1, year || undefined)
      ? `${year ? `${year}-` : ""}${String(month).padStart(2, "0")}-${String(
          date
        ).padStart(2, "0")}`
      : undefined;
  }
  ...
  return monthIndex !== -1 && isValidDate(Number(date), monthIndex)
    ? `${String(monthIndex + 1).padStart(2, "0")}-${String(date).padStart(
        2,
        "0"
      )}`
    : undefined;
